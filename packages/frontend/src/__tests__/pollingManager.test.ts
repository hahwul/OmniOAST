...
vi.mock('@/utils/time', () => ({
  ...
  toNumericTimestamp: (ts: number | string) => (typeof ts === 'number' ? ts : 1234567890),
}));
...
describe('usePollingManager', () => {
  ...
  describe('resume (Non-Interactsh Provider)', () => {
    it('should poll events for non-interactsh provider', async () => {
      ...
      const tabId = 'tab-1';
      const provider = { id: providerId, name: 'BOAST', type: 'BOAST', url: 'http://boast.url', token: 'token' };
      ...
      expect(mockUpdatePollingLastPolled).toHaveBeenCalledWith(pollingId, expect.any(Number));
      expect(mockSetPollingRunning).toHaveBeenCalledWith(pollingId, true);
      expect(mockRegisterPollingStop).toHaveBeenCalledWith(pollingId, expect.any(Function));
    ...
    it('should handle provider not found', async () => {
      ...
      expect(mockShowToast).toHaveBeenCalledWith(expect.stringContaining('Provider not found'), expect.anything());
  ...
  describe('resume (Interactsh Provider)', () => {
    it('should start interactsh client (fresh start)', async () => {
      ...
      const provider = { id: providerId, name: 'Interactsh', type: 'interactsh', url: 'https://interact.sh', token: '' };
      ...
      expect(mockUpdatePolling).toHaveBeenCalledWith(pollingId, expect.objectContaining({
  ...
  describe('resumeAll', () => {
      it('should resume all stopped tasks', async () => {
          ...
          mockOastStore.pollingList = [
              { id: pollingId1, providerId: 'p1', provider: 'BOAST', interval: 5000, tabId: 't1', payload: 'pl1' },
              { id: pollingId2, providerId: 'p2', provider: 'BOAST', interval: 5000, tabId: 't2', payload: 'pl2' }
          ];
