...
async function getPayload() {
    ...
    if (oastStore.activeTab) {
        await oastStore.addPayloadToHistory(
            oastStore.activeTab.id,
            finalPayload,
        );
    }
                            ...
                            await tmpClient.start(
                                {
                                    ...
                                    sessionInfo: {
                                        ...
                                        token: task.session.token,
                                        privateKey:
                                            task.session.privateKey || "",
                                        correlationID:
                                            task.session.correlationID,
                                        secretKey: task.session.secretKey,
                                        publicKey: task.session.publicKey,
                                    },
                                },
            ...
            <div class="px-4 pb-2 flex-shrink-0">
                ...
                <div class="flex flex-wrap gap-1.5">
                    ...
                    >
                        ...
                        >
                            <span class="truncate max-w-[320px]" :title="p">{{
                                p
                            }}</span>
                            <Button
