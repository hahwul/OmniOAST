...
const filteredInteractions = computed(() => {
    const activeInteractions = oastStore.interactions[oastStore.activeTabId] || [];
    return activeInteractions.filter((i) => {
        const matchesSearch =
...
const onTabChange = (event: any) => {
    if (event.index === oastStore.tabs.length) {
        oastStore.addTab();
    } else {
        const tab = oastStore.tabs[event.index];
        if (tab) {
            oastStore.setActiveTab(tab.id);
        }
    }
};
...
const activeTabIndex = computed({
    get: () => {
        const index = oastStore.tabs.findIndex(t => t.id === oastStore.activeTabId);
        return index === -1 ? 0 : index;
    },
    set: (newIndex) => {
        const tab = oastStore.tabs[newIndex];
        if (tab) {
            oastStore.setActiveTab(tab.id);
        }
    }
...
<template>
    <div class="h-full flex flex-col gap-1 omnioast-tab-view">
        <TabView v-model:activeIndex="activeTabIndex" @tab-change="onTabChange" class="flex-grow flex flex-col">
            <TabPanel v-for="(tab) in oastStore.tabs" :key="tab.id">
                <template #header>
                    <div class="flex items-center gap-2">
                        ...
                        <Button 
                            ...
                            @click.stop="oastStore.removeTab(tab.id)"
                            v-if="oastStore.tabs.length > 1"
                        />
                    </div>
                </template>
...
<style>
.omnioast-tab-view .p-tabview-nav {
    flex-shrink: 0;
}
.omnioast-tab-view .p-tabview-panels {
    flex-grow: 1;
    overflow: auto;
}
.omnioast-tab-view .p-tabview-panel {
    height: 100%;
}
.omnioast-table .p-datatable-tbody > tr > td {
    padding: 0.5rem 1rem;
}
...
.oast-search-bar:hover {
    border-color: var(--primary-color);
}

.oast-search-bar:focus {
    outline: 0 none;
    outline-offset: 0;
