...
describe("Backend Initialization", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(providerServiceModule.getProviderService).mockReturnValue(
      providerServiceMock as any,
    );
    ...
    vi.mocked(settingsServiceModule.getSettingsService).mockReturnValue(
  ...
  it("should initialize services", () => {
    init(mockSdk);
    expect(providerServiceModule.initProviderService).toHaveBeenCalledWith(
      mockSdk,
    );
    expect(settingsServiceModule.initSettingsService).toHaveBeenCalledWith(
      mockSdk,
    );
  });
  ...
  describe("Provider API Registration", () => {
    it("should register createProvider and call service", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "createProvider",
      )?.[1];
    ...
    it("should register updateProvider and call service with array args", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "updateProvider",
      )?.[1];
    ...
    it("should register deleteProvider and call service with array args", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "deleteProvider",
      )?.[1];

      // Call with (event, [id])
      callback("event", ["test-id"]);
      expect(providerServiceMock.deleteProvider).toHaveBeenCalledWith(
        "test-id",
      );
    });
    ...
    it("should register toggleProviderEnabled and call service with array args", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "toggleProviderEnabled",
      )?.[1];
  ...
  describe("OAST API Registration", () => {
    it("should register registerAndGetPayload and call service", async () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "registerAndGetPayload",
      )?.[1];
      const mockProvider = { type: "BOAST" };
      const mockOASTService = {
        registerAndGetPayload: vi
          .fn()
          .mockResolvedValue({ id: "1", payloadUrl: "url" }),
      };
      ...
      expect(providerServiceMock.getOASTService).toHaveBeenCalledWith(
        mockProvider,
      );
  ...
  describe("Settings API Registration", () => {
    it("should register createSettings and call service with parsed args", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "createSettings",
      )?.[1];
    ...
    it("should register getSettings and call service", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "getSettings",
      )?.[1];

      // Call with (event, id)
      callback("event", "settings-id");
      expect(settingsServiceMock.getSettings).toHaveBeenCalledWith(
        "settings-id",
      );
    });
    ...
    it("should register getCurrentSettings and call service", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "getCurrentSettings",
      )?.[1];
    ...
    it("should register updateSettings and call service with array args", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "updateSettings",
      )?.[1];
    ...
    it("should register deleteSettings and call service", () => {
      ...
      const callback = registerMock.mock.calls.find(
        (call: any[]) => call[0] === "deleteSettings",
      )?.[1];

      // Call with (event, id)
      callback("event", "settings-id");
      expect(settingsServiceMock.deleteSettings).toHaveBeenCalledWith(
        "settings-id",
      );
    });
